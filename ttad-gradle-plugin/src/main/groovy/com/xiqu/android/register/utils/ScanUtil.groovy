...
class ScanUtil {
    ...
    static boolean shouldProcessClass(String entryName) {
        def includePattern = RegisterTransform.inputPattern
        if (includePattern != null) {
            return includePattern.matcher(entryName).find()
        } else {
            return entryName != null && entryName.startsWith(ScanSetting.SCAN_CLASS_PACKAGE_NAME)
        }
    }
    ...
    static void scanClass(File file) {
        if (shouldProcessClass(file.name)) {
